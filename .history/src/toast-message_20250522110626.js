export class ToastMessage extends HTMLElement {
  ...
  connectedCallback() {
    ...
    // Calcular la posición antes de añadir al array
    const spacing = 10;
    const height = 80;
    const currentToasts = ToastMessage.toasts[position];
    const isTop = position.startsWith('top');
    const offset = currentToasts.length * (height + spacing);
    const translateY = isTop ? offset : -offset;
    ...
    if (position.includes('left')) this.style.left = '20px';
      ...
      this.style.left = '50%';
      this.style.transform = `translateX(-50%) translateY(${translateY}px)`;
    }

    // Añadir al array después de configurar la posición
  ...
  updateToastPositions(position) {
    ...
    toasts.forEach((toast, i) => {
      ...
      if (position.includes('center')) {
        toast.style.transform = `translateX(-50%) translateY(${isTop ? offset : -offset}px)`;
      } else {
